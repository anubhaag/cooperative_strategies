...
def Multi_HRES(x,y,P_mgs,bid,ask,b_grid,s_grid):
    ...
    m.Obj(np.sum(Loss_sbp)+np.sum(Loss_s1p)+np.sum(Loss_s2p)+np.sum(Loss_b1p) + np.sum(Loss_b2p) )
    m.Obj(0.05*(-np.sum(ub11) + np.sum(us12) - np.sum(ub0)- np.sum(us0)))
...
for i in range(T+1):
    if (P_loss[i]>P_loss_un[i])or(TMV_un[i]>TMV[i]):
        P_loss[i] = None
        TMV[i] = None
        Total_c[i] = None
        P_ms[i] = P_gridT[:,i]
